...
export default async function scrapMovieDatesAndTimes(targetDate: string): Promise<Schedule | []> {
    ...
    try {
        ...
        const response = await page.goto("http://www.cgv.co.kr/ticket/?MOVIE_CD=20033175&MOVIE_CD_GROUP=20033175&THEATER_CD=0013", { waitUntil: "networkidle0" });
        ...
        console.error(`${new Date().toLocaleString()} - An error occured: ${error}`);
    } finally {
        browser.close();
    }
    return result;
